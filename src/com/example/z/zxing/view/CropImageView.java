...
public class CropImageView extends ImageView {
	...
	private final int EDGE_L = 8;
	private final int EDGE_T = 9;
	private final int EDGE_R = 10;
	private final int EDGE_B = 11;
	...
	public int getTouch(int eventX, int eventY) {
		if (mFloatDrawable.getBounds().left <= eventX
			...
			return EDGE_RB;
				...
				&& (mFloatDrawable.getBounds().left + mFloatDrawable.getBorderWidth() > eventX)
				&& (mFloatDrawable.getBounds().top + mFloatDrawable.getBorderHeight()) < eventY 
				&& (mFloatDrawable.getBounds().bottom - mFloatDrawable.getBorderHeight()) > eventY){
			return EDGE_L;
		} else if ((mFloatDrawable.getBounds().left + mFloatDrawable.getBorderHeight()) <= eventX 
				&& (mFloatDrawable.getBounds().right - mFloatDrawable.getBorderWidth() > eventX)
				&& mFloatDrawable.getBounds().top < eventY 
				&& (mFloatDrawable.getBounds().top + mFloatDrawable.getBorderHeight()) > eventY){
			return EDGE_T;
		} else if ((mFloatDrawable.getBounds().right - mFloatDrawable.getBorderWidth()) <= eventX 
			...
			return EDGE_R;
				...
				&& (mFloatDrawable.getBounds().right - mFloatDrawable.getBorderWidth() > eventX)
				&& mFloatDrawable.getBounds().bottom > eventY 
				&& (mFloatDrawable.getBounds().bottom - mFloatDrawable.getBorderHeight()) <= eventY){
			return EDGE_B;
	...
	protected void checkBounds() {
		...
		if (mDrawableFloat.top < getTop()+dipTopx(mContext, 50)) {
			newTop = getTop()+dipTopx(mContext, 50);
			isChange = true;
		}
	...
	public Bitmap getCropImage() {
		...
		Bitmap ret = Bitmap.createBitmap(getDrawingCache(), mDrawableFloat.left, mDrawableFloat.top, mDrawableFloat.width(), mDrawableFloat.height(), matrix, true);
		// tmpBitmap.recycle();
		// tmpBitmap = null;
		return ret;
	}
